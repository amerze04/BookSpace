...
// there is no Booking to point at. That kind is anchored to
// RecurrenceRuleId + OccurrenceDate instead. Exactly one anchor is always
// set, enforced by CK_Notifications_HasContext in the DB and mirrored below.
//
...
public class Notification
{
    ...
    private Notification(
        ...
        DateTime nowUtc)
    {
        // CK_Notifications_HasContext: exactly one anchor, never neither
        if (bookingId is null && (recurrenceRuleId is null || occurrenceDate is null))
            throw new ArgumentException("A notification must reference either a Booking or a RecurrenceRule + OccurrenceDate.");
