...
public class ArchiveResourceCommandRequestHandlerTests
{
    ...
    private static ArchiveResourceCommandRequestHandler Handler(FakeResourceRepository repository) =>
    ...
    public async Task Handle_ArchivesTheResourceAndTouchesTheAuditColumns()
    {
        ...
        var response = await Handler(repository).Handle(
            new ArchiveResourceCommandRequest(resource.Id), CancellationToken.None);
    ...
    public async Task Handle_UnknownResource_ThrowsResourceNotFound()
    {
        ...
        var exception = await Assert.ThrowsAsync<ResourceNotFoundException>(() =>
            Handler(repository).Handle(new ArchiveResourceCommandRequest(Guid.NewGuid()), CancellationToken.None));
    ...
    // UpdateResourceCommandRequestHandlerTests, where editing an archived resource is
    ...
    public async Task Handle_AlreadyArchived_DoesNotTouchTheAuditColumns()
    {
        ...
        await Handler(repository).Handle(new ArchiveResourceCommandRequest(resource.Id), CancellationToken.None);
    ...
    {
        var resource = ExistingResource();
        var handler = new ArchiveResourceCommandRequestHandler(
            new FakeResourceRepository(resource),
            new FixedCurrentUser(null),
            new TestClock(NowUtc));

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            handler.Handle(new ArchiveResourceCommandRequest(resource.Id), CancellationToken.None));
    }
}
