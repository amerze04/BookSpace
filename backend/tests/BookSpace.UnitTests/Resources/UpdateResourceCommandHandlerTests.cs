...
public class UpdateResourceCommandHandlerTests
{
    ...
    private static UpdateResourceCommand Command(
        Guid resourceId,
    ...
    private static UpdateResourceCommandHandler Handler(FakeResourceRepository repository) =>
        new(
            repository,
    ...
    public async Task Handle_AppliesEveryFieldAndTouchesTheAuditColumns()
    {
        ...
        Assert.Equal("Board Room", response.Resource.Name);
        Assert.Equal("Top floor", response.Resource.Description);
        Assert.Equal("MeetingRoom", response.Resource.ResourceType);
        Assert.Equal(12, response.Resource.Capacity);
        Assert.Equal(15, response.Resource.MinDurationMinutes);
        Assert.Equal(60, response.Resource.MaxDurationMinutes);
    ...
    public async Task Handle_NullsClearRatherThanPreserve()
    {
        ...
        Assert.Null(response.Resource.Description);
        Assert.Null(response.Resource.MinDurationMinutes);
        Assert.Null(response.Resource.MaxDurationMinutes);
    }
    ...
    public async Task Handle_RequiresApprovalWithAnApprover_Succeeds()
    {
        ...
        Assert.True(response.Resource.RequiresApproval);
        Assert.Equal(1, repository.SaveChangesCount);
    }
    ...
    public async Task Handle_CapacityDecreaseToExactlyTheCommittedUnits_Succeeds()
    {
        ...
        Assert.Equal(5, response.Resource.Capacity);
    }
    ...
    public async Task Handle_CapacityIncrease_DoesNotQueryExistingBookings()
    {
        ...
        Assert.Equal(20, response.Resource.Capacity);
