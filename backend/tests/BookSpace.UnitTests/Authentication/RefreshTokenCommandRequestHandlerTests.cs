...
// RefreshTokenCommandRequestHandler, because the difference between the rows (revoke
// one token vs. revoke the whole family) is the entire security property.
public class RefreshTokenCommandRequestHandlerTests
{
    ...
    private sealed class Harness
    {
        ...
        public RefreshToken Replacement(RefreshTokenCommandResponse result) =>
            RefreshTokens.Tokens.Single(t => t.TokenHash == TokenFactory.HashOf(result.RefreshToken));

        public Task<RefreshTokenCommandResponse> Handle(string rawToken)
        {
            var issuer = new TokenIssuer(new FakeAccessTokenService(), TokenFactory, RefreshTokens, _clock);
            var handler = new RefreshTokenCommandRequestHandler(
                Users,
                RefreshTokens,
                TokenFactory,
                issuer,
                _clock,
                NullLogger<RefreshTokenCommandRequestHandler>.Instance);

            return handler.Handle(new RefreshTokenCommandRequest(rawToken), CancellationToken.None);
        }
    }
}
